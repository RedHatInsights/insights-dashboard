...
const getAdvisorZeroState = () => ({
  header: {
    description: `Using Red Hat’s expertise, analyze your RHEL hosts to identify and resolve risks to your environment's availability, performance, and stability.`,
    commands: [
...
const getComplianceZeroState = () => ({
  header: {
    ...
    commands: [
      ...
      {
        plainText:
          ' 4. Create a Compliance Policy and assign your systems to it',
      },
...
const getMalwareZeroState = () => ({
  header: {
    description:
      'The malware detection service monitors your RHEL hosts for known malware signatures to indicate potential threats you can proactively address with your information security team.',
    commands: [
      { plainText: ' 1. Register your host' },
      {
      ...
      {
        singleClipboardCommand:
          'yum -y install insights-client # necessary for some versions of RHEL 8',
      },
      {
        singleClipboardCommand: 'insights-client --register',
      },
      ...
      {
        instructions: '2. Install the yara package',
        command: 'dnf -y install yara',
        noPadding: true,
      },
      {
...
const getResourceOptimizationZeroState = () => ({
  header: {
    ...
    commands: [
      ...
      { plainText: 'or' },
      {
        instructions: 'Complete the manual install',
        link: 'https://docs.redhat.com/en/documentation/red_hat_lightspeed/1-latest/html/assessing_and_monitoring_rhel_resource_optimization_with_red_hat_lightspeed/assembly-ros-install#proc-installing-ros_assembly-ros-install',
      },
      { plainText: '2. Register your host' },
      {
...
const getImagesZeroState = () => ({
  header: {
    description: `Create customized images optimized for your target environment and save time by uploading to multiple target environments in a single build.`,
    bulletPoints: [
    ...
    commands: [
      {
        plainText:
          ' Images is supposed to have a custom input via customButton prop',
      },
    ],
  },
  otherApps: [
...
const getRemediationPlansZeroState = () => ({
  header: {
    ...
    commands: [
      ...
      {
        plainText:
          ' 2. For RHEL 8 and 9 hosts, install and connect with remote host configuration (rhc)',
      },
