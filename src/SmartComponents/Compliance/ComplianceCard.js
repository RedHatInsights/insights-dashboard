...
const ComplianceCard = ({
  ...
  const getTopThreeReports = function (compliance) {
    const complianceTopThree =
      compliance.data.length > 1
        ? compliance.data.slice(0, 3)
  ...
  return (
    <ExpandableCardTemplate
      ...
      isExpanded={JSON.parse(
        localStorage.getItem('dashboard_expanded_compliance') || 'true',
      )}
      isExpandedCallback={(isExpanded) =>
        localStorage.setItem('dashboard_expanded_compliance', isExpanded)
      }
      header={<TemplateCardActions />}
      body={
        <TemplateCardBody>
            ...
            <React.Fragment>
              {complianceFetchStatus === 'fulfilled' &&
                ...
                (complianceSummary.data.length > 0 ? (
                  <React.Fragment>
                    <DataList
                      className="insd-m-no-padding insd-m-no-top-border"
                      isCompact
                    >
                      {getTopThreeReports(complianceSummary).map(
                        (report, index) => (
                  ...
                  <EmptyState
                    className="ins-c-compliance__empty-state"
                  ...
                  >
                    ...
                    <EmptyStateActions>
                      ...
                      >
                        {intl.formatMessage(
                          messages.complianceEmptyStateAction1,
                        )}
                      </InsightsLink>
                      <Button
                        variant="link"
                      ...
                      >
                        {intl.formatMessage(
                          messages.complianceEmptyStateAction2,
                        )}
                      </Button>
                    </EmptyStateActions>
                  </EmptyState>
...
export default connect(mapStateToProps, mapDispatchToProps)(ComplianceCard);
