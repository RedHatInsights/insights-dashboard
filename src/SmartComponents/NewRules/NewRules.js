...
const NewRules = () => {
  ...
  const [isExpanded, setIsExpanded] = useState(
    JSON.parse(localStorage.getItem('dashboard_expanded_cta') || 'true'),
  );
  const vulnerabilities = useSelector(
    ({ DashboardStore }) => DashboardStore.vulnerabilities,
  );
  let { recent_rules: newRules } = vulnerabilities;

  return (
    <DataList
      className="insd-c-dashboard-data-list insd-m-toggle-right-on-md insd-m-no-border pf-m-compact"
      gridBreakpoint="none"
    >
            ...
            dataListCells={[
              <DataListCell key="primary content">
                ...
                >
                  ...
                  >
                    {intl.formatMessage(messages.latestCritical)}
                  </span>
                </Flex>
              </DataListCell>,
            ]}
                ...
                <Flex direction={{ default: 'column' }} flex={{ md: 'flex_3' }}>
                  <FlexItem spacer={{ default: 'spacerXs' }}>
                    <Title
                      headingLevel="h4"
                      size="xl"
                      className="pf-v6-u-font-weight-lights"
                    >
                      <span>
                        {capitalize(
                          intl.formatMessage({
                    ...
                    <DescriptionListGroup>
                      ...
                      {item.associated_cves.map((cve) => (
                        <DescriptionListDescription key={cve.key}>
                          <InsightsLink app="vulnerability" to={`/cves/${cve}`}>
                            {cve}
                          </InsightsLink>
                        </DescriptionListDescription>
                      ))}
                  ...
                  <Flex flex={{ md: 'flex_1', xl: 'flexDefault' }}>
                    {item.node_id && (
                      <a
                        href={`https://access.redhat.com/node/${item.node_id}`}
                        rel="noreferrer"
                        target="_blank"
                      >
