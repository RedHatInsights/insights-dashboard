...
import {
  patchmanFetchAdvisories,
  patchmanFetchSystems,
} from '../../AppActions';
...
const PatchManagerCard = ({
  ...
  const intl = useIntl();
  const isLoaded = [systemsStatus, advisoriesStatus].every(
    (item) => item === 'fulfilled',
  );
  const {
    security,
  ...
  const pieChartData = [
    {
      x: intl.formatMessage(messages.securityAdvisories, { count: security }),
      y: security,
      fill: chart_color_blue_400.value,
      url: `/advisories?offset=0&filter%5Badvisory_type_name%5D=security`,
    },
    {
      x: intl.formatMessage(messages.bugfixAdvisories, { count: bugs }),
      y: bugs,
      fill: chart_color_blue_300.value,
      url: `/advisories?offset=0&filter%5Badvisory_type_name%5D=bugfix`,
    },
    {
      x: intl.formatMessage(messages.enhancementAdvisories, {
        count: enhancements,
      }),
      y: enhancements,
      fill: chart_color_blue_200.value,
  ...
  if (systemsStatus === 'rejected') {
    return (
      ...
      >
        <TemplateCardHeader
          subtitle={intl.formatMessage(messages.patchTitle)}
        />
        <TemplateCardBody>
          <FailState appName="Patch" isSmall />
        </TemplateCardBody>
      </TemplateCard>
    );
  }
  ...
  return (
    <ExpandableCardTemplate
      ...
      isExpanded={JSON.parse(
        localStorage.getItem('dashboard_expanded_patch') || 'true',
      )}
      isExpandedCallback={(isExpanded) =>
        localStorage.setItem('dashboard_expanded_patch', isExpanded)
      }
        ...
        <TemplateCardBody>
          {!isLoaded ? (
            ...
            <Flex direction={{ default: 'column' }}>
              <InsightsLink
                app="patch"
                to="/systems?filter[stale]=false"
                className="pf-v6-c-button pf-m-link pf-m-inline"
              >
                <span>
                  {intl.formatMessage(messages.systemsAffected, {
                    count: systems,
                  })}
                </span>
              </InsightsLink>
                    ...
                    {pieChartData.map((item, index) => (
                      ...
                      >
                        <span
                          ...
                          style={{
                            '--insd-c-legend__dot--BackgroundColor': `${colorScale[index]}`,
                          }}
                        />
                        <span className="insd-c-legend__text">
                          {item.y} {capitalize(item.x)}
                        </span>
                      </InsightsLink>
                    ))}
                  </div>
