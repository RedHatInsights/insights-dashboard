...
const workloadsPropType = (props, propName, componentName) => {
  ...
  if (typeof props !== 'object') {
    error = new Error(
      `\`${componentName}\` only accepts object as \`${propName}\` prop.`,
    );
  }
  ...
  if (keys.some((key) => !SAP_KEYS.includes(key))) {
    error = new Error(
      `\`${componentName}\` accepts either SAP or All workloads as \`${propName}.\` prop.`,
    );
  }

  if (keys.find((key) => SAP_KEYS.includes(key))?.length > 1) {
    error = new Error(
      `\`${componentName}\` accepts only one of SAP or All workloads as \`${propName}.\` prop.`,
    );
  }

  const values = Object.values(prop);
  const foundIncorrect = values.findIndex(
    (value) => typeof value?.isSelected !== 'boolean',
  );
  if (foundIncorrect !== -1) {
    error = new Error(
      `\`${componentName}\` requires isSelected as boolean prop in \`${propName}.${keys?.[foundIncorrect]}\`.`,
    );
  }
  ...
  generateFilter(
    {
      system_profile: {
        ...
        ...(workloads?.['Microsoft SQL']?.isSelected && { mssql: 'not_nil' }),
      },
    },
    undefined,
...
const supportsGlobalFilter = (selectedTags, workloads) =>
  !Object.values(workloads || {})
    .map((value) => value?.isSelected)
    .reduce((res, cur) => res || cur, false) && selectedTags.length === 0;
