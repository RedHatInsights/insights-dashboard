...
import {
  INVENTORY_FETCH_URL,
  INVENTORY_STALE_FETCH_URL,
  INVENTORY_WARNING_FETCH_URL,
} from '../AppConstants';
...
export const useBatchInventoryFetch = (workloads, selectedTags, hasAccess) => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setIsLoading(true);
        const inventorySum = await axios.get(`${INVENTORY_FETCH_URL}${tags}`, {
          params,
        });
        const inventoryWarningSum = await axios.get(
          `${INVENTORY_WARNING_FETCH_URL}${tags}`,
          { params },
        );
        const inventoryStaleSum = await axios.get(
          `${INVENTORY_STALE_FETCH_URL}${tags}`,
          { params },
        );
